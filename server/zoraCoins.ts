...
const DEFAULT_FACTORY = "0x777777751622c0d3258f214f9df38e35bf45baf3";

const FACTORY_ADDRESS = (process.env.ZORA_FACTORY_ADDRESS || DEFAULT_FACTORY).toLowerCase();
const LOOKBACK_BLOCKS = Number.parseInt(process.env.ZORA_FACTORY_LOOKBACK_BLOCKS || "200000", 10);
const BLOCK_RANGE = Number.parseInt(process.env.ZORA_FACTORY_BLOCK_RANGE || "2000", 10);
const REFRESH_TTL = Number.parseInt(process.env.ZORA_FACTORY_REFRESH_TTL_MS || "30000", 10);
...
const manualTokens = (process.env.ZORA_MINTED_TOKENS || "")
  .split(",")
...
export async function getZoraCoinSet() {
  await refreshCoins();
  return state.coins;
}

export function getZoraCoinStatus() {
  return {
